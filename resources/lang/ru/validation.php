<?php


return [
    'user' => [
      'name' => [
          'required' => 'Имя обязательно',
          'max'      => 'Максимальное количество символов должно не превышать 255',
          'min'      => 'Минимальное количество символов должно быть больше 6',
      ],
      'email' => [
          'required'     => 'Почта обязательна',
          'unique'       => 'Почта уже юзается',
          'email'        => 'Почта имеет неверный формат',
          'invalid_code' => 'Передайте верификационый код'
      ],
      'password' => [
          'required'  => 'Пароль обязателен',
          'confirmed' => 'Подтверждение пароля обязательно'
      ],
      'phone' => [
          'required' => 'Передайте телефон',
          'regex'    => 'Неверный формат телефона(Прим: 80(25)8965390)'
      ],
      'role' => [
          'in'       => 'Передана несуществующая роль',
          'required' => 'Передайте роль',
      ],
      'params' => [
          'required' => 'Передайте параметры',
          'name' => [
              'required' => 'Передайте название параметра',
              'in'       => 'Передано несуществующее название параметра'
          ],
          'value' => [
              'required' => 'Передайте значение параметра',
              'string'   => 'Значение параметра должно быть строкой',
              'integer'  => 'Значение параметра должно быть числом'
          ],
          'type' => [
              'in' => 'Нету типа для данного параметра'
          ]
      ]
    ],
    'disciplines' => [
        'number_of_practices' => [
            'required' => 'Передайте количество практик',
            'value'    => 'Неверное число'
        ],
        'number_of_labs' => [
            'required' => 'Передайте количество лаб',
            'value'    => "Неверное число"
        ],
        'name' => [
            'required' => 'Передайте имя',
            'string'   => 'Имя должно быть строкой'
        ],
        'department_id' => [
            'required' => 'Передайте отделение',
            'exists'   => 'Передано несуществующее отделение'
        ],
        'description' => [
            'required' => 'Передайте описание',
            'string'   => 'Описание должно быть строкой'
        ]
    ],
    'departments' => [
        'name' => [
            'required' => 'Передайте имя отделения'
        ],
        'manager_id' => [
            'required' => 'Передайте мэнэджера'
        ],
        'description' => [
            'required' => 'Передайте описание'
        ]
    ],
    'groups' => [
        'name' => [
            'required' => 'Передайте название',
            'string'   => 'Имя должно быть строкой'
        ],
        'description' => [
            'required' => 'Передайте описание'
        ],
        'discipline_id' => [
            'required' => 'Передайте дисциплину',
            'in'       => 'Передана несуществующая дисциплина'
        ],
        'students' => [
            'number' => [
                'required' => 'Передайте номер студента',
                'in' => 'Передан несуществующий номер студента',
                'is' => 'Студент уже добавлен к вам в группу'
            ],
            'student_id' => [
                'required' => 'Передайте идентификатор студента'
            ]
        ],
        'tasks' => [
            'name' => [
                'required' => 'Передайте название',
                'string' => 'Название должно быть строкой'
            ],
            'description' => [
                'required' => 'Передайте описание'
            ],
            'expired_at' => [
                'required' => 'Передайте дату окончания',
                'data' => 'Неверный формат даты'
            ]
        ]
    ]
];
